...
(defn get-arch-name
  ...
  (let [properties (get-system-properties)
        normal-os (if (:os.name properties) 
                      (clojure.string/lower-case (clojure.string/replace (:os.name properties) 
                                                                         #"\s+" "-"))
                      "")
        normal-arch (if (:os.arch properties) 
                        (clojure.string/lower-case (:os.arch properties)) 
                        "")]
...
(defn load-resource
  ...
  (let [arch-name (get-arch-name)
        home-path (get-home-path)
        ds (get-file-separator)
        cache-path (clojure.string/join ds 
                                        [home-path 
                                         ".clj-nativedep" 
                                         app-id 
                                         app-version 
                                         arch-name ])
        cache-file (java.io.File. (clojure.string/join ds 
                                                       [cache-path 
                                                        (if (win?) 
                                                            (str lib-name 
                                                                 ".dll") 
        ...
        cache-lib-mod-date (if (.exists cache-file) 
                               (java.util.GregorianCalendar.))]
    (try (let [p (java.io.File. cache-path)]
              (if (.exists p)
                  nil
                  (.mkdirs p)))
              (if cache-lib-mod-date
                  (.setTimeInMillis cache-lib-mod-date 
                                    (.lastModified cache-file)))
              (if pkg-lib-mod-date
                  (.setTimeInMillis pkg-lib-mod-date 
                                    (.getLastModified resource-conn)))
              ; check if file in cache
              (if (and pkg-lib-mod-date
                       (or (not cache-lib-mod-date)
                           (> (.compareTo cache-lib-mod-date 
                                          pkg-lib-mod-date) 
                              0)))
                  (let [buffer (byte-array 1024)
                        in (.getInputStream resource-conn)
                        out (java.io.FileOutputStream. cache-file)]
                       (try (loop [bc (.read in buffer)]
                              (if (< bc  0)
                                  nil
                                  (recur (do (.write out buffer 0 bc)
                                             (.read in buffer)))))
                            (finally (try (.close in)
              ...
              (finally (if resource-conn
                           (try (.close resource-conn)
                                (catch Throwable t nil)))))))
